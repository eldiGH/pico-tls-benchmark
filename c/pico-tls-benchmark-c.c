...
void make_https_request_test(const char *hostname, const char *url, const char *cert_type)
{
    ...
    printf("Request to https://%s%s SUCCEEDED in %.2fms | tcp_connect time: %.2fms | tls_open_connection time: %.2fms | cert: %s\n",
           hostname,
           url,
           (float)(time_us_64() - req_start_time) / 1000,
           (float)(state->tls_state->tcp_state->tcp_connect_end_time - state->tls_state->tcp_state->tcp_connect_start_time) / 1000,
           (float)(state->tls_state->handshake_end_time - state->tls_state->handshake_start_time) / 1000,
           cert_type);
    https_free_state(state);
    return;
}
...
void make_http_request_test(const char *hostname, const char *url)
{
    ...
    if (err != 0)
    {
        printf("Request to http://%s%s FAILED in %.2fms with error number: %d\n", hostname, url, (float)(time_us_64() - req_start_time) / 1000, err);
        http_free_state(state);
        return;
    }

    printf("Request to http://%s%s SUCCEEDED in %.2fms| tcp_connect time: %.2fms\n",
           hostname,
           url,
           (float)(time_us_64() - req_start_time) / 1000,
           (float)(state->tcp_state->tcp_connect_end_time - state->tcp_state->tcp_connect_start_time) / 1000);
    http_free_state(state);
    return;
}
...
int main()
{
    ...
    make_http_request_test("www.google.com", "/"); // http variant just for good measure, it's the only site that allows http from urls used here.
